...
// 1. Add customFormFields to db.json
const dbPath = './backend/db.json';
const db = require(dbPath);
if (!db.customFormFields) {
  db.customFormFields = [];
...
if (!serverCode.includes('/api/admin/form-fields')) {
  const apiCode = `
...
app.listen(PORT`;
  serverCode = serverCode.replace('app.listen(PORT', apiCode);
  fs.writeFileSync('backend/server.js', serverCode, 'utf8');
}

// 3. Create AdmissionFormBuilderModule.tsx
                ...
                {fields.map((f, index) => (
                  <tr key={f.id || index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 text-right space-x-2">
                      ...
                      <button onClick={() => handleDelete(f.id)} className="inline-flex items-center justify-center w-8 h-8 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-colors border border-red-200 rounded-sm" title="Delete"><Trash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  <form id="fieldForm" onSubmit={handleSave} className="space-y-6">
                    ...
                    <div>
                      ...
                      <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full border border-gray-300 p-3 text-sm focus:border-emerald-600 focus:outline-none font-bold rounded-sm">
...
`;
fs.writeFileSync('frontend/src/components/admin/AdmissionFormBuilderModule.tsx', moduleCode, 'utf8');

console.log('Backend & Admin Module created');
