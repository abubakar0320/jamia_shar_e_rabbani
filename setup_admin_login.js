...
// 1. Backend: Add login route
const serverPath = path.join(__dirname, 'backend', 'server.js');
let serverCode = fs.readFileSync(serverPath, 'utf8');

if (!serverCode.includes('/api/admin/login')) {
  const loginRoute = `
...
`;
  serverCode = serverCode.replace('app.listen(PORT', loginRoute + '\napp.listen(PORT');
  fs.writeFileSync(serverPath, serverCode, 'utf8');
}

// 2. Frontend: Create AdminGuard Component
const guardContent = `'use client';
...
`;
fs.writeFileSync(path.join(__dirname, 'frontend/src/components/admin/AdminGuard.tsx'), guardContent, 'utf8');

// 3. Wrap Admin layout with AdminGuard
const layoutPath = path.join(__dirname, 'frontend/src/app/admin/layout.tsx');
let layoutCode = fs.readFileSync(layoutPath, 'utf8');

if (!layoutCode.includes('AdminGuard')) {
...
console.log('Admin Security Implementation Complete!');
